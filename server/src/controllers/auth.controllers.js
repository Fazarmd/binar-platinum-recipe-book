...
function generateAccessToken(user) {
  const accessToken = jwt.sign({ user: { user_id: user.id } }, JWT_ACCESS_SECRET, {
    expiresIn: "1h",
  });
...
function generateRefreshToken(user) {
  const refreshToken = jwt.sign({ user: { user_id: user.id } }, JWT_REFRESH_SECRET, {
    expiresIn: "7d",
  });
  return refreshToken;
}
...
async function loginAndStoreTokens(req, res) {
  ...
  try {
    ...
    if (userByUsername && (await bcrypt.compare(password, userByUsername.hash_password))) {
      ...
      console.log("Login acces token:", access_token);
      req.access_token = access_token;
      req.refresh_token = refresh_token;

      return res.status(201).json(responseOk("Login Successfully", { access_token, refresh_token }));
    } else {
...
async function refreshAccessToken(req, res) {
  ...
  jwt.verify(refresh_token, JWT_REFRESH_SECRET, (err, user) => {
    ...
    return res.status(200).json(responseOk("Access Token Refreshed", { access_token }));
  });
}
